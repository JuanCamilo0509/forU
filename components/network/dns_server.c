...
static const char *DNS = "example_dns_redirect_server";
...
static int parse_dns_request(char *req, size_t req_len, char *dns_reply,
  ...
  for (int qd_i = 0; qd_i < qd_count; qd_i++) {
    ...
    ESP_LOGD(DNS, "Received type: %d | Class: %d | Question for: %s", qd_type,
             qd_class, name);
    ...
    if (qd_type == QD_TYPE_A) {
      ...
      ESP_LOGD(DNS, "Answer with PTR offset: 0x%" PRIX16 " and IP 0x%" PRIX32,
  ...
  while (handle->started) {
    ...
    while (handle->started) {
      ...
      // Data received
        ...
        ESP_LOGI(DNS, "Received %d bytes from %s | DNS reply with len: %d", len,
                 addr_str, reply_len);
        if (reply_len <= 0) {
...
dns_server_handle_t start_dns_server(dns_server_config_t *config) {
  ...
  ESP_RETURN_ON_FALSE(handle, NULL, DNS,
